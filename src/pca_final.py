...
# ─────────────────────────────────────────────────────────────────────
atomic_radii      = {'Ni': 124,   'Ti': 147,   'Hf': 159  }   # pm, clementicovalent
...
waber_cromer = {'Ni': 1.563, 'Ti': 2.086, 'Hf': 2.325} # Waber-Cromer pseudopotential radii (Å) — Waber & Cromer 1965
#metallic_radius = {'Ni': 124, 'Ti': 147, 'Hf': 159} # Metallic radius (pm) — measured in metallic crystal
R = 8.314  # J/(mol·K)
...
def add_alloy_features(df: "pd.DataFrame") -> "pd.DataFrame":
    ...
    #df['Weighted_Mr (pm)'] = sum(c[e] * metallic_radius[e] for e in els)
...
def drop_redundant(feature_list):
    return [f for f in feature_list if f not in DROP_FEATURES]

all_features_clean = drop_redundant(all_features_clean)
...
print(f"Clean — dropped {len(dropped_clean)} collinear features: {dropped_clean}")
all_features_clean = list(kept_clean)   # update the list so biplots still work

corr_df = pd.DataFrame(X_scaled_clean, columns=all_features_clean).corr()
high_corr = [
...
def drop_redundant(feature_list):
    return [f for f in feature_list if f not in DROP_FEATURES]
all_features_imp   = drop_redundant(all_features_imp)
...
print(f"Imputed — dropped {len(dropped_imp)} collinear features: {dropped_imp}")
all_features_imp = list(kept_imp)

corr_df = pd.DataFrame(X_scaled_imp, columns=all_features_clean).corr()
high_corr = [
    (c1, c2, round(corr_df.loc[c1, c2], 4))
    for i, c1 in enumerate(corr_df.columns)
    for c2 in corr_df.columns[i+1:]
...
for c1, c2, r in high_corr:
    print(f"  {c1}  ↔  {c2}  r={r}")
corr_df = pd.DataFrame(X_scaled_imp, columns=all_features_imp).corr()
high_corr = [
    (c1, c2, round(corr_df.loc[c1, c2], 4))
    for i, c1 in enumerate(corr_df.columns)
    for c2 in corr_df.columns[i+1:]
...
print(f"\nImputed  — PC1+PC2: {pca_imp.explained_variance_ratio_[:2].sum():.1%}")
print(f"Clean    — PC1+PC2: {pca_clean.explained_variance_ratio_[:2].sum():.1%}")
print(pca_clean.explained_variance_ratio_)
#verification block
for col in ['Weighted_dor (Å)', 'Weighted_Atomic_Radius (pm)']:
    in_clean   = col in X_clean.columns
    in_imputed = col in X_imputed.columns
    print(f"{col:40s}  clean={in_clean}  imputed={in_imputed}")
...
feature_cols_c = list(X_clean.columns)
scale        = 4
...
COMPOSITION_FEATURES = {
    ...
    'Weighted_MeltingPoint (K)',        # legacy name if present
    'Weighted_MeltingPoint_Mr (K)',     # new canonical name
    'Weighted_Period',                  # legacy name if present
    'Weighted_Period (dor)',            # new canonical name
...
all_features_clean = available_features + engineered_features
feature_cols_c = all_features_clean

for i, feature in enumerate(feature_cols_c):
...
all_features_imp = available_features + engineered_features
feature_cols_e = all_features_imp
...
# ─────────────────────────────────────────────────────────────────────
# CLUSTER INTERPRETATION — run after KMeans for both paths
# ─────────────────────────────────────────────────────────────────────
def interpret_clusters(cluster_summaries, label=""):
    print(f"\n{'═'*60}")
    print(f"  Cluster interpretation — {label}")
    print(f"{'═'*60}")
    ...
    for k, cs in cluster_summaries.items():
        ...
        # Hf content is the primary discriminator in NiTiHf
        if hf < 5:
            family = "Binary-like  (Hf-lean,  NiTi base)"
        elif hf < 15:
            family = "Low-Hf       (dilute ternary)"
        elif hf < 25:
            family = "Mid-Hf       (moderate substitution)"
        else:
            family = "High-Hf      (heavy substitution)"

        # Ms interpretation
        if np.isnan(ms):
            ms_note = "Ms unknown"
        elif ms < 0:
            ms_note = f"Ms={ms:.0f}°C  (cryogenic, austenite at RT)"
        elif ms < 100:
            ms_note = f"Ms={ms:.0f}°C  (near-RT transformation)"
        elif ms < 300:
            ms_note = f"Ms={ms:.0f}°C  (elevated Ms, actuator range)"
        else:
            ms_note = f"Ms={ms:.0f}°C  (high-Ms, HTSMA territory)"

        rows.append((k+1, n, ni, ti, hf, ms_note, family))

    # Sort by Hf content so the table reads low→high substitution
    rows.sort(key=lambda r: r[4])

    print(f"{'C':>2}  {'n':>4}  {'Ni':>6}  {'Ti':>6}  {'Hf':>6}  "
          f"{'Ms note':<38}  Family")
    print(f"{'─'*2}  {'─'*4}  {'─'*6}  {'─'*6}  {'─'*6}  "
          f"{'─'*38}  {'─'*30}")
    for r in rows:
        print(f"{r[0]:>2}  {r[1]:>4}  {r[2]:>6.1f}  {r[3]:>6.1f}  {r[4]:>6.1f}  "
              f"{r[5]:<38}  {r[6]}")

    print()
    # Flag any cluster where Ms variance is high — means KMeans split
    # on composition but Ms is not monotonic within cluster (worth noting)
    print("  Monotonicity check (Hf↑ should correlate Ms↑ in NiTiHf):")
    hf_vals = [r[4] for r in rows]
    ms_raw  = [cs['ms'] for cs in cluster_summaries.values()]
    # re-sort ms by hf order
    paired  = sorted(zip(hf_vals, ms_raw))
    ms_sorted = [p[1] for p in paired]
    monotone  = all(
        np.isnan(ms_sorted[i]) or np.isnan(ms_sorted[i+1]) or
        ms_sorted[i] <= ms_sorted[i+1]
        for i in range(len(ms_sorted)-1)
    )
    print(f"  {'OK — Ms increases with Hf content' if monotone else 'WARNING — Ms not monotone with Hf; check cluster validity'}")

# Run for both paths
interpret_clusters(cluster_summaries,   label="Graph D — clean data")
interpret_clusters(cluster_summaries_e, label="Graph E — imputed data")
# ─────────────────────────────────────────────────────────────────────
